...
interface UseRotaPorVozReturn {
  ...
  tentarProcessarTranscricoes: (transcricoes: readonly string[]) => boolean;
}
...
export default function useRotaPorVoz({
  ...
  const resolverReferencia = useCallback(
    (referencia: ReferenciaFilialVoz): number | null => {
      ...
      if (referencia.tipo === 'posicao') {
        return rotas[referencia.posicao - 1]?.codigofilial ?? null;
      }
  ...
  const informarReferenciaAusente = useCallback((): void => {
    responder(
      'Não consegui identificar a parada. Fale o número da filial ou a posição na rota.',
    );
  }, [responder]);
  ...
  /**
   * Executa apenas quando o interpretador local reconhece a intenção. Essa
   * variante permite tentar um provedor externo antes de exibir o fallback.
   */
  const tentarProcessarTranscricoes = useCallback(
    (transcricoes: readonly string[]): boolean => {
      const {comando} = selecionarComandoVozRota(
        transcricoes,
        codigosValidos,
      );

      if (comando.tipo === 'nao_reconhecido') return false;

      processarTranscricoes(transcricoes);
      return true;
    },
    [codigosValidos, processarTranscricoes],
  );

  return {
    processarTranscricoes,
    tentarProcessarTranscricoes,
  };
}
