...
const ACAO_ADICIONAR =
  /\b(adiciona|adicionar|adicione|coloca|colocar|coloque|comeca|comecar|comece|inclui|incluir|inclua|ir|leva|levar|leve|passe|passar|preciso|quero|termina|terminar|termine|vamos|visitar|visite|vou)\b/;
const ACAO_REMOVER =
...
const criarReferencia = (trecho: string): ReferenciaFilialVoz | null => {
  const posicaoPorExtenso = Object.entries(POSICOES_POR_EXTENSO).find(
    ([termo]) => new RegExp(`\\b${termo}\\b`).test(trecho),
  )?.[1];
  const posicaoNumerica = trecho.match(
    /\b(?:parada|posicao|lugar)\s+(\d+)\b/,
  )?.[1];

  if (posicaoPorExtenso !== undefined || posicaoNumerica !== undefined) {
    return {
      tipo: 'posicao',
      posicao: posicaoPorExtenso ?? Number(posicaoNumerica),
    };
  }
...
export const interpretarComandoVozRota = (transcricao: string): ComandoVozRota => {
  ...
  if (ACAO_REMOVER.test(texto)) {
    ...
    if (alvo?.tipo === 'posicao') {
      return {tipo: 'remover_referencia', alvo};
    }
  ...
  if (
    codigos.length > 0 &&
    (
      ACAO_ADICIONAR.test(texto) ||
      CONTEXTO_FILIAL.test(texto) ||
      CONTEXTO_SEQUENCIA.test(texto) ||
      somenteNumero
    )
  ) {
