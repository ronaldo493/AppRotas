...
const normalizarCategoria = (valor: unknown): string =>
  String(valor ?? '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
const classificarCategoria = (valor: unknown): string => {
  const categoria = normalizarCategoria(valor);

  if (categoria.includes('restaurante')) return 'restaurante';
  if (categoria.includes('posto') || categoria.includes('combustivel')) {
    return 'posto';
  }
...
export const encontrarPontosProximos = (
  ...
  pontos
    .filter(
      ponto =>
        classificarCategoria(ponto.categoria) === classificarCategoria(categoria),
    )
    .map(ponto => {
