...
export default function useAssistenteFalante(): UseAssistenteFalanteReturn {
  ...
  const falar = useCallback(async (texto: string): Promise<void> => {
    ...
    if (versaoFala !== versaoFalaRef.current || !respostasFaladasAtivas) {
      return;
    }

    /*
     * A descoberta de vozes pode ser lenta em alguns aparelhos. A primeira
     * resposta usa imediatamente a voz padrão e prepara a melhor voz em
     * segundo plano para as próximas falas.
     */
    ...
    Speech.speak(texto, {
      language: 'pt-BR',
      /* Ritmo próximo da fala cotidiana, ainda legível para ramais e rotas. */
      rate: 1.1,
      pitch: 1,
      voice: voz ?? undefined,
    });
