...
import type { SugestaoTipo } from '../../type/FeedBack';
import { useAppTheme } from '../ThemeStyles';
import styles from './styles/SugestionStyles';
import useSugestionPosition from '../../hooks/useSugestionPosition';

const SUGESTION_POSITION_KEY = '@drogal:sugestion-fab-position';
...
export default function Sugestion(): React.JSX.Element {
  const { colors } = useAppTheme();
  const insets = useSafeAreaInsets();

  const { loading, enviarSugestao } = useSugestao();

  const { panHandlers, transform } = useSugestionPosition(SUGESTION_POSITION_KEY);
